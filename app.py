...
def showItem(category_name, item_name):
    ...
    if 'username' not in login_session or \
    creator.id != login_session['user_id']:
        return render_template('public_itemdetail.html', item=item,
...
@app.route('/catalog/<path:category_name>/delete', methods=['GET',
           'POST'])
@login_required
def deleteCategory(category_name):
    ...
    if creator.id != login_session['user_id']:
        flash('You cannot delete it. This is belongs to %s'
              % creator.name)
        return redirect(url_for('showCatalog'))
    if request.method == 'POST':
...
def deleteItem(category_name, item_name):
    ...
    # check logged in user not equal owner redirect them
